...
def assign_to_cluster(row):
    euclidean = 10000
    cent = 0
    coordinates = np.array([row['ppg'], row['atr']])
                           
    for key, value in centroids_dict.items():
        values = np.array(value)
        distance = euclidean_distances(values.reshape(1,-1), coordinates.reshape(1,-1))
...
def recalculate_centroids(df):
    ...
    for cluster_id in range(0, num_clusters):
        ...
        new_centroids_dict[cluster_id] = [np.average(players['ppg']), np.average(players['atr'])]
